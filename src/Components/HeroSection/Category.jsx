...
import { VscDebugRestart } from "react-icons/vsc"; // أيقونة الريسيت
...
const Category = () => {
  ...
  // دالة إعادة الضبط (Reset)
  const handleResetFilters = () => {
    setSelectedBrands([]);
    setMinPrice(0);
    // نترك maxPrice كما هي لأنها تأتي من الـ API
  ...
  // 2. جلب البيانات المفلترة مع "تنظيف" الـ Body
  const { data: filteredData, isFetching: isFiltering } = useQuery({
    ...
    queryFn: async () => {
      ...
      // إضافة الماركات فقط إذا لم تكن فارغة
      if (selectedBrands.length > 0) filters.brands = selectedBrands;

      // تنظيف الخصائص: إرسال الخصائص التي تحتوي على قيم مختارة فقط
              ...
              <div className="flex justify-between items-center mb-6 border-b pb-4">
                ...
                {hasActiveFilters && (
                  <button
                    ...
                    className="text-red-500 text-sm flex items-center gap-1 hover:underline transition-all"
                  >
          ...
          <div className="lg:col-span-3">
            ...
            {displayProducts.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {displayProducts.map(
                  ({ images, name, slug, variants, id, base_price }) => (
                    <div
                      key={id}
                      className="group bg-white rounded-[2.5rem] p-4 transition-all hover:shadow-xl border border-transparent hover:border-gray-100"
                    >
                      <div className="relative overflow-hidden rounded-3xl bg-gray-50 h-[200px]">
                        ...
                        <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                          <button
                            ...
                            className="w-10 h-10 bg-white rounded-full flex items-center justify-center hover:bg-red-500 hover:text-white transition-all"
                          >
                            <AiOutlineHeart size={18} />
                          </button>
                          <Link
                            to={`/productDetails/${slug}`}
                            className="w-10 h-10 bg-white rounded-full flex items-center justify-center hover:bg-main hover:text-white transition-all"
                          >
                      ...
                      <div className="mt-4 px-2">
                        ...
                        <button
                          ...
                          className="w-full bg-[#F3F6FF] text-[#000E39] py-3 rounded-xl font-bold hover:bg-main hover:text-white transition-all flex items-center justify-center gap-2"
                        >
              ...
              <div className="text-center py-20 bg-white rounded-[3rem]">
                ...
                <button
                  onClick={handleResetFilters}
                  className="mt-4 text-main underline"
                >
                  عرض كل المنتجات
                </button>
              </div>
